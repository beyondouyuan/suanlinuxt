<template>
  <div class="trade-container">
    <section class="tab-container">
      <div class="tab-box">
        <span class="tab-item" :tagId="item.id" :class="[contract_id == item.id ? 'active' : '']" v-for="(item, index) of contracts" :key="index" @click="handleSwitchTag(item.id)">{{item.contract_name}}</span>
      </div>
    </section>
    <section class="trade-content-container">
      <div class="info-wrapper">
        <div class="bar-panel">
          ...
          <div class="panel-item contract-link">
            <nuxt-link :to="{name: 'contract-id', params: {id: contract_id}}">合约详情&gt;&gt;</nuxt-link>
          </div>
        </div>
      </div>
      ...
      <section class="history-box">
        ...
        <div v-if="c_num_pages > 1" class="pagination-container">
          <pagination :page-index="c_currentPage" :total="c_count" :page-size="pageSize" @change="c_pageChange">
          </pagination>
        </div>
      </section>
      <section class="history-box">
...
export default {
  ...
  data() {
    return {
      seo: {
        title: '交易市场-算力多云算力投资平台',
        keywords: '云算力,算力,矿机,云算力租赁,云算力出租,云算力租用,算力租赁,算力出租,算力租用,矿机托管,矿机出租,矿机租赁,矿机租用',
        desc: '算力多是目前全球领先的比特币挖矿、莱特币云挖矿算力投资平台之一。致力于提供算力租赁,云算力出租,云算力租用,算力租赁,算力出租,算力租用,矿机托管,矿机出租,矿机租赁,矿机租用等服务的云算力投资平台。'
      },
      contracts: [],
      currentContract: {},
      FormInObject: {},
  ...
  async asyncData({ params, req, res, error }) {
    const { data: detail } = await requestContractDetail()
    const historyParamas = {
      page: 1,
      contract_id: 1,
      status: ['deal']
    }
    const notDealParamas = {
      contract_id: 1,
      status: ['not_deal', 'part_deal']
    }
    const bookParams = {
      contract_id: 1,
      num: 6
    ...
    const [
      ...
      requestExchangeList(historyParamas),
      requestExchangeList(notDealParamas),
      requestExchangeOrderbook(bookParams)
    ]).catch(err => {
      console.log(err)
    })
    const { list_of_trade, current_page, num_pages } = history
    let data = { orderbook }
    data.ask = orderbook.data.ask.reverse()
    data.bid = orderbook.data.bid
    const { pc, phs } = assets
    return {
      contracts: detail,
      contract_id: detail[0].id,
      currentContract: detail[0],
      newestData: newest,
      list_of_trade: list_of_trade,
      currentPage: current_page,
      num_pages: num_pages,
      count: 15 * num_pages,
      list_of_current: notDeal.list_of_current,
      c_currentPage: notDeal.current_page,
      c_num_pages: notDeal.num_pages,
      c_count: 15 * notDeal.num_pages,
      priceData: data,
      FormInObject: {
        title: '买入',
        type: 'limit',
...
<style lang="scss">
@import '../../assets/styles/trading.scss';
